...
void bridge_upd_hw_forwarding(const struct ifnet *ifp)
{
	/*
	...
	if (ifp->if_brport) {
		if (bridge_port_get_bridge(ifp->if_brport) == ifm) {
			if (bridge_port_get_state(ifp->if_brport) != state) {
				...
				fal_br_upd_port(ifindex, &attr_list[0]);
			}
		} else {
			DP_DEBUG(BRIDGE, ERR, BRIDGE,
...
bridge_fdb_flush(struct ifnet *bridge, struct ifnet *ifp,
		 uint8_t fdb_type, uint16_t vlanid)
{
...
void bridge_fdb_dynamic_flush_vlan(struct ifnet *bridge, struct ifnet *port,
				   uint16_t vlanid)
{
	bridge_fdb_flush(bridge, port, IFBAF_DYNAMIC, vlanid);
}
...
static void bridge_delport(int ifindex, int ifmaster)
{
	...
	ifpromisc(ifp, 0);
	bridge_fdb_flush(ifm, ifp, IFBAF_ALL, 0);
	fal_br_del_port(ifmaster, ifindex);
}
...
bridge_netlink_update_port(int ifindex, struct nlattr *tb[], int msg_type)
{
	...
	struct ifnet *port = ifnet_byifindex(ifindex);
	if (!port)
		return rv;
				  ...
				  void *arg)
{
	...
	ifp = ifnet_byifindex(child_ifindex);
	if (!ifp)
		RTE_LOG(ERR, BRIDGE,
...
bridge_macs_clear(FILE *f, int argc, char **argv, struct ifnet *bridge)
{
	...
	if (macp) {
		...
		bridge_fdb_flush(bridge, port, IFBAF_DYNAMIC, 0);
	}
	return 0;
}
