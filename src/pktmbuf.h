...
struct pkt_mdata_flow {
	struct ip_addr nexthop;
	vrfid_t ingress_vrf_id;
	uint32_t input_ifindex;
} __attribute__ ((__packed__));
...
enum pkt_mdata_invar_type {
	PKT_MDATA_INVAR_FLOW		= (1 << 0),
	PKT_MDATA_INVAR_ALT_FLOW	= (1 << 1),
	PKT_MDATA_INVAR_SPATH		= (1 << 2),
	/*
	 ...
	 */
	PKT_MDATA_INVAR_BRIDGE		= (1 << 3),
	PKT_MDATA_INVAR_NAT64		= (1 << 4),
};
...
struct pktmbuf_mdata {
	/* PKT_MDATA_INVAR_FLOW */
	struct flow_data *md_flowp;
...
pktmbuf_mdata(struct rte_mbuf *m)
{
 ...
 */
#define pktmbuf_mtol3(m, t) ((t)(rte_pktmbuf_mtod(m, char *) +	\
				 (m)->l2_len))
 ...
 *   The type to cast the result into.
 */
#define pktmbuf_mtol4(m, t) ((t)(rte_pktmbuf_mtod(m, char *) + \
				 (m)->l2_len + (m)->l3_len))
 ...
 *   The packet mbuf.
 */
#define pktmbuf_l2_len(m) ((m)->l2_len)
 ...
 *   The packet mbuf.
 */
#define pktmbuf_l3_len(m) ((m)->l3_len)
 ...
 *
 * The source mbuf has its metadata cleared. It is expected that the
