...
static void
pl_show_ipv4_route_lookup(struct vrf *vrf, struct pl_show_vrf_ctx *ctx)
{
	...
	pl_node_iter_features(ipv4_route_lookup_node_ptr, vrf,
			      pl_print_feats, wr);
	jsonw_end_array(wr);
...
/*
 * show features ipv4_route_lookup [vrf <vrfname | default | all>]
 */
static int cmd_pl_show_feat_ipv4_route_lookup(struct pl_command *cmd)
{
	int argc = cmd->argc;
	char **argv = cmd->argv;
	...
	VRF_FOREACH(vrf, vrf_id)
		pl_show_ipv4_route_lookup(vrf, &ctx);
...
PL_REGISTER_OPCMD(pl_show_feat_ipv4_route_lookup) = {
	.cmd = "show features ipv4_route_lookup",
	.handler = cmd_pl_show_feat_ipv4_route_lookup,
};
