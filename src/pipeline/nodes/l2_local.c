 ...
 */
uint16_t l2_local_features;

static inline struct pl_node *l2_local_feat_list_to_node(void)
{
	/* our imaginary node */
	return (struct pl_node *)&l2_local_features;
}

static inline uint16_t *
drop_node_to_l2_local_feat_list(struct pl_node *node)
{
...
l2_local_process_common(struct pl_packet *pkt, void *context __unused,
			 enum pl_mode mode)
{
	switch (mode) {
	case PL_MODE_FUSED:
		if (!pipeline_fused_l2_local_features(
			    pkt, l2_local_feat_list_to_node()))
			return L2_LOCAL_FINISH;
		break;
	case PL_MODE_FUSED_NO_DYN_FEATS:
		if (!pipeline_fused_l2_local_no_dyn_features(
			    pkt, l2_local_feat_list_to_node()))
			return L2_LOCAL_FINISH;
		break;
	case PL_MODE_REGULAR:
		if (!pl_node_invoke_enabled_features(
			    l2_local_node_ptr,
			    l2_local_feat_list_to_node(),
			    pkt))
			return L2_LOCAL_FINISH;
		break;
	}

	local_packet_internal(pkt->in_ifp, pkt->mbuf);
	return L2_LOCAL_ACCEPT;
}

ALWAYS_INLINE unsigned int
l2_local_process(struct pl_packet *p, void *context)
{
	return l2_local_process_common(p, context, PL_MODE_REGULAR);
}

static int
l2_local_feat_change(struct pl_node *node,
		      struct pl_feature_registration *feat,
		      enum pl_node_feat_action action)
{
	uint16_t *feature_list = drop_node_to_l2_local_feat_list(node);

	return pl_node_feat_change_u16(feature_list, feat, action);
}

ALWAYS_INLINE bool
l2_local_feat_iterate(struct pl_node *node, bool first,
...
l2_local_node_lookup(const char *name)
{
	if (strcmp(name, "all") == 0)
		return l2_local_feat_list_to_node();
...
PL_REGISTER_NODE(l2_local_node) = {
	...
	.feat_change = l2_local_feat_change,
	.feat_iterate = l2_local_feat_iterate,
	.lookup_by_name = l2_local_node_lookup,
	.num_next = L2_LOCAL_NUM,
	.next = {
		[L2_LOCAL_ACCEPT] = "term-finish",
		[L2_LOCAL_FINISH] = "term-finish",
	}
};
