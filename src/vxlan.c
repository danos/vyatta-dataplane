			  ...
			  struct ip_addr *nhip)
{
	...
	/* Lookup destination */
	nxt = rt_lookup(dip->address.ip_v4.s_addr, RT_TABLE_MAIN, m);
			  ...
			  struct ip_addr *nhip)
{
	...
	nxt6 = rt6_lookup(&dip->address.ip_v6, RT_TABLE_MAIN, m);
	if (unlikely(nxt6 == NULL))
		return -ENOENT;

	dif = nh6_get_ifp(nxt6);
	if (unlikely(dif == NULL))
		return -ENOENT;
				  ...
				  struct ifnet *dif)
{
	if (likely(dip->type == AF_INET)) {
		...
		if (!ip_l2_resolve_and_output(ifp, m,
					      &nh, ETH_P_IP)) {
		...
		if (!ip6_l2_resolve_and_output(ifp, m, &nh, ETH_P_IPV6)) {
			     ...
			     struct nlattr *tb[], uint flags __unused)
{
	...
	if (vxlaninfo[IFLA_VXLAN_LINK]) {
		...
		struct ifnet *pifp = ifnet_byifindex(pifi);
	    ...
	    enum cont_src_en cont_src)
{
	...
	if (tb[IFLA_LINK]) {
		...
		if (iflink != 0) {
			struct ifnet *pifp = ifnet_byifindex(iflink);
	     ...
	     enum cont_src_en cont_src, const struct nlmsghdr *nlh)
{
	...
	if (if_link) {
		...
		if_idx = cont_src_ifindex(cont_src,
					  ifi->ifi_index);
		if (link_idx != 0) {
			struct ifnet *pifp = ifnet_byifindex(link_idx);
		       ...
		       struct nlattr *tb[])
{
	...
	case RTM_NEWNEIGH:
		...
		struct ifnet *ifp = ifnet_byifindex(ndm->ndm_ifindex);
...
{
	int ret = if_register_type(IFT_VXLAN, &vxlan_if_ops);
	if (ret < 0)
