...
#define MACVLAN_MODE_VRRP 32	/* pass all packets that aren't vrrp  */
	       ...
	       bool return_parent_if)
{
	...
	if (node) {
		...
		if (mvle->mode == MACVLAN_MODE_VRRP && return_parent_if)
...
macvlan_add_mac(struct ifnet *ifp, struct ether_addr *eth_addr)
{
	MVL_DEBUG("%s adding %s to port %d\n", ifp->if_name,
		ether_ntoa(eth_addr), ifp->if_port);
	if_add_l2_addr(ifp, eth_addr);
}

static void
macvlan_del_mac(struct ifnet *ifp, struct ether_addr *eth_addr)
{
	MVL_DEBUG("%s deleting %s from port %d\n", ifp->if_name,
		ether_ntoa(eth_addr), ifp->if_port);
	if_del_l2_addr(ifp, eth_addr);
}
	       ...
	       const struct ether_addr *eth_addr, int ifindex)
{
	...
	if (vifp) {
		...
		/* For now default to VRRP */
		mvle->mode = MACVLAN_MODE_VRRP;
...
void macvlan_flood(struct ifnet *ifp, struct rte_mbuf *m)
{
	...
	cds_lfht_for_each_entry(mvlt->mvlt_hash, &iter, mvle, mvl_node) {
		if (mvle->mode != MACVLAN_MODE_VRRP) {
...
struct ifnet *macvlan_get_vrrp_ip_if(struct ifnet *ifp, in_addr_t target)
{
	struct mvl_tbl *mvlt = rcu_dereference(ifp->if_macvlantbl);
	struct mvl_entry *mvle;
	struct cds_lfht_iter iter;
	...
	cds_lfht_for_each_entry(mvlt->mvlt_hash, &iter, mvle, mvl_node) {
		...
		cds_list_for_each_entry_rcu(ifa, &mvle->ifp->if_addrhead,
			...
			if (sa->sa_family != AF_INET)
				continue;
			if (target != satosin(sa)->sin_addr.s_addr)
				continue;
