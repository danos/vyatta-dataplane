...
gre_tunnel_add_tracker(struct gre_info_st *greinfo, struct vrf *vrf)
{
	...
	greinfo->ti_info = rt_tracker_add(vrf, &addr, greinfo,
...
gre_tunnel_remove_tracker(struct gre_info_st *greinfo, struct vrf *vrf)
{
	...
	rt_tracker_delete(vrf, &addr, greinfo);
}
			       ...
			       struct ip_addr *ip)
{
	...
	case AF_INET:
		...
		ret = nh_lookup_by_index(ti_info->nhindex, hash,
	...
	case AF_INET6:
		...
		ret = nh6_lookup_by_index(ti_info->nhindex, hash,
					  &ip->address.ip_v6,
					  &nh_ifindex);
	...
	nh_ifp = ifnet_byifindex(nh_ifindex);
	if (!nh_ifp) {
		RTE_LOG(ERR, GRE,
			      ...
			      char **argv)
{
	...
	if (argc == 1) {
		ifnet_walk(tun_neighbor_dump, json);
		goto end;
	}
...
static void tun_show_tracker(json_writer_t *json, struct gre_info_st *greinfo)
{
	...
	if (greinfo->family == AF_INET) {
		hash = ecmp_iphdr_hash(&greinfo->iph, 0);
		ret = nh_lookup_by_index(greinfo->ti_info->nhindex,
					 hash, &nh, &nh_ifindex);
		if (ret != 0)
			goto unresolved;

		nifp = ifnet_byifindex(nh_ifindex);
		if (!nifp)
			goto unresolved;
...
static void
gre_if_pre_delete(struct ifnet *ifp)
{
	/* Delete the tunnel object */
	gre_tunnel_remove_tep(ifp, NULL);
	if_delete_l3_intf(ifp);
}

static void
gre_if_create_finished(struct ifnet *ifp,
		       const struct ether_addr *mac_addr)
{
	if_create_l3_intf(ifp, mac_addr);
...
static const struct ift_ops gre_if_ops = {
	.ifop_pre_uninit = gre_if_pre_delete,
	.ifop_uninit = gre_tunnel_delete,
	.ifop_dump = gre_tunnel_dump,
	.ifop_create_finished = gre_if_create_finished,
};
