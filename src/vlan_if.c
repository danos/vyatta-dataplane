...
static void
vlan_if_create_finished(struct ifnet *ifp,
			const struct ether_addr *mac_addr)
{
	if (!ifp->if_parent ||
	    (ifp->if_parent->if_type != IFT_BRIDGE))
		return;
...
static void
vlan_if_pre_delete(struct ifnet *ifp)
{
...
static const struct ift_ops vlan_if_ops = {
	...
	.ifop_pre_uninit = vlan_if_pre_delete,
	...
	.ifop_create_finished = vlan_if_create_finished,
};
				       ...
				       __unused const void *l2_addr)
{
	...
	ifnet_walk(vlan_callback_mac_addr_changed, ifp);
}
