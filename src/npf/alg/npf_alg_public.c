		...
		const struct ifnet *ifp, const int di, int *error)
{
	...
	apt = alg_get_apt(ai, npf_cache_ipproto(npc));
	if (!apt)
		return NULL;
	 ...
	 * wildcarded sport
	 */
	nt = alg_search_all_then_any_sport(apt, npc, ifp);
	if (!nt)
		return NULL;
	 ...
	 */
	if (unlikely(!nt->nt_paired)) {
		if (!(nt->nt_flags & NPF_TUPLE_MULTIMATCH))
			apt_expire_tuple(nt);
	} else {
		rte_spinlock_lock(&apt->apt_lock);
		if (nt->nt_flags & NPF_TUPLE_EXPIRED)
			do_drop = true;
		npf_alg_tuple_expire_pair(nt);
		rte_spinlock_unlock(&apt->apt_lock);
	}
...
{
	const struct npf_alg *npf_alg = npf_alg_session_get_alg(se);
