...
static int ftp_alg_tuple_insert(const struct npf_alg *ftp,
				 ...
				 uint32_t alg_flags, struct npf_alg_nat *an)
{
	...
	if (nt) {
		...
		nt->nt_dport = dport;
		nt->nt_sport = sport;
		nt->nt_timeout = FTP_TUPLE_TIMEOUT;
		nt->nt_dstip = *daddr;
		nt->nt_srcip = *saddr;
		rc = npf_alg_tuple_add_replace(ftp->na_ai, nt);
		if (rc) {
			RTE_LOG(ERR, FIREWALL, "FTP: tuple insert:%d\n", rc);
...
{
	const struct npf_alg *ftp = npf_alg_session_get_alg(parent);
	npf_addr_t oaddr;
	in_port_t tmp;
		...
		struct npf_alg_tuple *nt, const int di)
{
	...
	case NPF_TUPLE_MATCH_ANY_SPORT:
		rc = npf_alg_session_nat(se, npf_alg_parent_nat(nt->nt_se),
				npc, di, nt);
		if (!rc) {
			npf_alg_session_set_flag(se, nt->nt_alg_flags);
			npf_session_link_child(nt->nt_se, se);
			npf_alg_session_set_private(se, nt->nt_data);
			nt->nt_data = NULL;
		}
		break;
	}
