...
void dp_test_npf_debug(bool enable)
{
	...
	snprintf(cmd, TEST_MAX_CMD_LEN, "debug %snpf", enable ? "":"- ");
...
static bool
json_match_intf(json_object *jobj, void *arg)
{
	char *intf = arg;
	const char *ointf;

	if (!dp_test_json_string_field_from_obj(jobj, "interface", &ointf))
		return false;

	return strcmp(ointf, intf) == 0;
}
 ...
 */
json_object *
_dp_test_npf_json_get_rs_intf(json_object *jarray, const char *intf,
			      const char *file, int line)
{
	json_object *jobj;
	void *arg = (void *)intf;
	...
	jobj = dp_test_json_array_iterate(jarray, &json_match_intf, arg);
			       ...
			       const char *file, int line)
{
	...
	*packets = 0;
	...
	jgrp = _dp_test_npf_json_get_ruleset(rset->rstype, rset->attach_point,
					     rset->dir, rset->name,
					     debug, file, line);
	_dp_test_fail_unless(jgrp, file, line,
			     "Failed to find ruleset %s %s",
			     rset->rstype, rset->name);

	for (rule = rset->rules; rule && rule->rule != NULL; rule++) {
		jrule = dp_test_npf_json_get_rs_rule(jgrp, rule->rule);
		_dp_test_fail_unless(jrule, file, line,
				     "Failed to find rule %s %s %s",
				     rset->rstype, rset->name, rule->rule);

		if (!dp_test_json_int_field_from_obj(jrule, "packets",
						     (int *)&rpkts)) {
			_dp_test_fail(file, line,
				      "Failed to get pkts count "
				      "from rule %s %s %s",
				      rset->rstype, rset->name, rule->rule);
			return false;
		}
...
_dp_test_npf_rule_pkt_count(struct dp_test_npf_ruleset_t *rset,
			    const char *rule, uint *packets,
			    bool debug, const char *file, int line)
{
	json_object *jrule;
	bool rv;

	jrule = dp_test_npf_json_get_rule(rset->rstype, rset->attach_point,
					  rset->dir, rset->name, rule);
	_dp_test_fail_unless(jrule, file, line,
			     "Failed to find rule %s %s %s",
			     rset->rstype, rset->name, rule);

	rv = dp_test_json_int_field_from_obj(jrule, "packets",
					     (int *)packets);
...
static const char *npf_rct_desc(enum npf_rc_type rct)
{
	...
	case NPF_RCT_ACL4:
		return "IPv4 packet filter";
	case NPF_RCT_ACL6:
		return "IPv6 packet filter";
	case NPF_RCT_NAT64:
		return "NAT64";
	}
	return "NPF";
